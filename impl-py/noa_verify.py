...
def _is_strict_public_key(raw):
    ...
    §5.1.3 decoding + small-order rejection). Same rule, same order, as src/keys.ts
    isStrictEd25519PublicKeyBytes and the Go / Rust / C# verifiers:
    ...
    if x == 0 and x_sign == 1:
        return False
    if bytes(raw) in _SMALL_ORDER_PUBKEYS:
        return False
    return True
