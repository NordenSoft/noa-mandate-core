...
import { keyHolderRCases } from "../dist/test/helpers/ed25519-keyholder.js";
...
// ── low-order / non-canonical PUBLIC KEY consensus pin. Which encodings a library admits as a public key
// differs between libraries and versions (OpenSSL admits small-subgroup keys at key load), so both impls
// refuse the 8 canonical small-order point encodings AND any non-canonical y ≥ q encoding themselves. A keyring whose pubkey is a low-order point → both TAMPERED
...
// Python ground truth the other three runners are compared against.
{
  ...
  const cases = [
    ...
    ["mixed-order pubkey", "keyring-mixed-order.json"],
  ];
...
// ── Knockout proof (signature-R rule), TS + Python: a fresh key signs a genuine receipt with a chosen R
// at test time (S = r + k*a with its own private scalar; nothing committed as bytes). The control
// signature must verify; R = identity, R = identity with the sign bit and R = rB + a small-order point
// must be refused by both verifiers.
{
  const rk = generateKeyPair("r-rule-key");
  const seed = Buffer.from(rk.privateKey, "base64").subarray(16);
  const base = mk(0, null);
  const receipt = JSON.parse(JSON.stringify(base));
  receipt.sig.kid = rk.kid;
  receipt.chain.hash = "sha256:" + sha256Hex(receiptHashInput(receipt));
  const message = signingMessage(RECEIPT_SIG_DOMAIN, receiptHashInput(receipt));
  const cases = keyHolderRCases(seed, message);
  const rKeyring = { [rk.kid]: rk.publicKey };
  const rKeyringPath = join(dir, "r-rule-keyring.json");
  writeFileSync(rKeyringPath, JSON.stringify(rKeyring));
  const run = (label, sig, wantTs, wantPy) => {
    const signed = JSON.parse(JSON.stringify(receipt));
    signed.sig.value = sig.toString("base64");
    const p = join(dir, `r-rule-${label.replace(/[^a-z0-9]+/gi, "-")}.json`);
    writeFileSync(p, JSON.stringify([signed]));
    const ts = verifyChain(bytesOf(p), { keyring: b(rKeyring) }).status;
    const tsOk = ts === wantTs;
    console.log(`${tsOk ? "✓" : "✗"} signature-R rule: ${label} [TS verifyChain]: ${ts} (want ${wantTs})`);
    if (!tsOk) failures++;
    expect(`signature-R rule: ${label} [PY verifier]`, pyVerify([p, rKeyringPath]).code, wantPy);
  };
  run("control, ordinary R", cases.control, "VALID", 0);
  for (const [label, sig] of cases.refused) run(label, sig, "TAMPERED", 2);
}
