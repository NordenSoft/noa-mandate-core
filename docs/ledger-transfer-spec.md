...
binary32 rounds `16777217`, and a signed 32-bit integer overflows above 2 147 483 647.
...
vector, so a reordering of the §6 steps fails at least one of them. Cross-phase precedence vectors
between the first and the last member (`amount` and `unit`) pin that absent, `null`, empty and
wrong-type are judged inside each member's own step (§3.3): each of those four categories appears
against a malformed value, in both directions, so an implementation that checks one category for every
member in a separate pass before or after the per-member rules — presence first, as a JSON Schema
`required` pass does, type first, or formats first with missing members afterwards — fails at least
one of them. The corpus proves exactly these two properties about ordering and nothing broader.
...
own fixture, every reachable code occurs, and every adjacent refusal-order pair and every
cross-phase pair is pinned by a vector whose input really violates both rules. The fixture is synthetic by construction: accounts
