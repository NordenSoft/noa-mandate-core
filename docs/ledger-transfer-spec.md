...
the wire, and 15 digits stay below 2^53, so an implementation using float64, int64 or arbitrary
precision integers reads it exactly.
...
**Amounts.** The fixed 15-digit range keeps every amount exact in every common numeric type. An effect
owner still has to use checked integer arithmetic on the validated string (§12).
...
An implementation conforms to `noa.ledger.transfer/1` iff, over every vector in
...
vector whose binding it must equal.
...
rebuilds its canonical tuple, every canonical form equals the §4.3 template, and every reachable code
occurs. The fixture is synthetic by construction: accounts `acct-example-N`, ledgers
`ledger-example-N`, the testing code `XTS`, counting-pattern salts.
