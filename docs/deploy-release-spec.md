...
`.invalid`, so it can never resolve), an all-zero account identifier, and counting-pattern digests
that no content-addressed artifact produces. This matters because the fixture is the PREIMAGE of a
