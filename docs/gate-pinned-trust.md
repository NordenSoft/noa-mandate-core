...
- **Ed25519 keys:** accepted exactly when the decision verifier would accept them
  ...
  point on the curve, no x = 0 point spelled with the sign bit set (RFC 8032 §5.1.3), not
  small-order, and in the prime-order subgroup ([L]A is the identity, so a mixed-order key is
  refused).
