 ...
 * validates that signer at authenticated genTime against caller-supplied roots and CRLs. Missing
 * policy is an error; this package never falls back to structural success.
 *
...
// One authenticated success currently invokes exactly these five fixed OpenSSL operations:
// version, ts -verify, ts -reply -token_out, cms -verify, and verify. The CLI multiplies this
// constant by its admitted unique-anchor count, so adding another backend operation without also
// raising and reviewing the declared work bound fails closed.
export const OPENSSL_PROCESS_BUDGET_PER_STAMP = 5;
...
function parseUtcInstant(s) {
  ...
  if (year < 1970 || month < 1 || month > 12 || hour < 0 || hour > 23 || minute < 0 || minute > 59 || second < 0 || second > 59) {
    return undefined;
  }
  let maxDay = 31;
  ...
  if (!isFiniteNumber(parsed) || !isSafeInteger(parsed) || parsed < 0) return undefined;
  return parsed;
}
...
function validatePolicy(options, parsed) {
  ...
  if (clockNowMs > 9007199254740991 - snapshot.maxFutureSkewMs || genTimeMs > clockNowMs + snapshot.maxFutureSkewMs) {
    return { valid: false, result: failure("GENTIME_IN_FUTURE", "authenticated genTime exceeds the trusted verification instant and allowed future skew") };
  }
  ...
  return {
    ...
    executable,
    allowedPolicyOids,
    trustRoots: roots.bytes,
    crls: crls.bytes,
    untrustedCertificates: untrusted?.bytes,
    timeoutMs,
    attime: `${attime}`,
...
function classifyCertificateFailure(run, fallbackCode, fallbackReason) {
  ...
  if (strIncludes(out, "certificate revoked")) {
    return failure("SIGNER_CERTIFICATE_REVOKED", "the timestamp signer certificate is revoked at genTime");
  }
  if (strIncludes(out, "CRL has expired") || strIncludes(out, "unable to get certificate CRL") || strIncludes(out, "unable to get CRL")) {
    return failure("REVOCATION_EVIDENCE_UNAVAILABLE", "required current CRL evidence was unavailable at genTime");
  }
  if (strIncludes(out, "certificate has expired")) {
    return failure("SIGNER_CERTIFICATE_EXPIRED", "the timestamp signer certificate was expired at genTime");
  }
  if (strIncludes(out, "certificate is not yet valid")) {
    return failure("SIGNER_CERTIFICATE_NOT_YET_VALID", "the timestamp signer certificate was not yet valid at genTime");
  }
...
function runOpenSslSequence(paths, inspected, policy, resourceBudget) {
  ...
  const verifyArgs = addProviderArgs([
    ...
    "-CRLfile", paths.crls, "-crl_check_all", "-purpose", "timestampsign", "-attime", policy.attime,
  ...
  if (run.status !== 0) {
    return classifyCertificateFailure(
      ...
      "the signer chain and required CRL status did not verify as good at genTime",
    );
  }
  ...
  return {
    ...
    verification: {
      ...
      revocation: { mode: "crl-check-all", status: "GOOD", at: inspected.parsed.genTime },
    },
  };
}
