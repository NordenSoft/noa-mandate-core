...
test("isStrictEd25519PublicKey accepts exactly the keys verifyEd25519 accepts", () => {
  ...
  const candidates: Array<[string, unknown]> = [
    ...
    // Off-curve encodings are in their own test below, so a knockout can bind exactly that detector.
...
// ONE test holds every off-curve encoding. The knockout pair pinned-ed25519-on-curve +
// strict-ed25519-prime-order-subgroup removes both checks that refuse such an encoding and is credited
// only when THIS test fails ([proof: AA-STRICT-KEY-OFF-CURVE]); the partner's own mixed-order failure
// elsewhere cannot stand in for it.
test("[PROOF:AA-STRICT-KEY-OFF-CURVE] an encoding that names no curve point is not accepted as a key and verifies nothing", () => {
  const kp = generateKeyPair("k-off-curve");
  const msg = signingMessage("NOA-Test-v0.1-sig", "{}");
  const goodSig = signEd25519(kp.privateKey, msg);
  assert.equal(verifyEd25519(kp.publicKey, msg, goodSig), true, "control: the genuine key verifies its own signature");
  // RFC 8032 §5.1.3 step 3: these y have no x on the curve, so the encoding names no point at all.
  const offCurve: Array<[string, string]> = [
  ...
  for (const [name, key] of offCurve) {
    assert.equal(isStrictEd25519PublicKey(key), false, `consequence: ${name} must not be accepted as a key`);
    assert.equal(verifyEd25519(key, msg, goodSig), false, `${name}: a refused key verifies nothing`);
  }
});
