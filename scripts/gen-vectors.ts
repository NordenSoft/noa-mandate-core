...
//     non-canonical and small-order public-key encodings at key load (RFC 8032 §5.1.3 decoding +
//     small-order rejection) and a non-canonical signature scalar (S >= L, RFC 8032 §5.1.7). Each
//     keyring maps the corpus kid to one refused key; run with valid-chain.json the receipts are
//     genuine, so the only thing that can refuse them is key validation → TAMPERED in all five.
