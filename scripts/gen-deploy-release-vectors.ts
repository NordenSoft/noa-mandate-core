 ...
 * `example.invalid/…` (RFC 2606 reserves `.invalid`, so it can never resolve), an all-zero account
 * id, and counting-pattern digests that no content-addressed artifact produces. This matters
...
const PIN_PARAMS_HASH = "sha256:e8575695110b0734e79586f5e68a34bdd60be838c41e302b44ec1734d89a9c03";
...
function baseParams(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...
    targetAccount: "acct-000000000000-example",
    operation: "deploy",
    ...overrides,
  };
}
...
vectors.push(
  check({
    ...
    expect: {
      ...
      display: {
        ...
        Environment: "staging",
        Target: "acct-000000000000-example",
        Image: DIGEST_A,
      },
    },
  }),
  check({
    ...
    paramsText:
      `{"operation":"deploy","targetAccount":"acct-000000000000-example","imageDigest":"${DIGEST_A}",` +
...
const separations: Array<[string, string, Record<string, unknown>]> = [
  ...
  ["accept-distinct-target", "A different target account.", { targetAccount: "acct-0000-us-east-1" }],
...
vectors.push(
  check({
    ...
    paramsText: `{"repository":"example.invalid/example-service","commit":5.1e+39,"environment":"staging","imageDigest":"${DIGEST_A}","targetAccount":"acct-000000000000-example","operation":"deploy"}`,
  ...
  check({
    ...
    paramsText: `{"repository":"example.invalid/example-service","commit":${"1".repeat(40)},"environment":"staging","imageDigest":"${DIGEST_A}","targetAccount":"acct-000000000000-example","operation":"deploy"}`,
...
vectors.push(
  check({
    ...
    paramsText:
      ...
      `"environment":"staging","imageDigest":"${DIGEST_A}","targetAccount":"acct-000000000000-example","operation":"deploy"}`,
...
const out = {
  ...
  note:
    ...
    "BY CONSTRUCTION (RFC 2606 `.invalid` host, all-zero account, counting-pattern digests) — it " +
