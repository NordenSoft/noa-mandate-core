...
`9` KEY_RETIRED (authentic signature by a lifecycle-retired key; refused for current use, see
below). Historical mode additionally uses `7` for an authenticated checkpoint conflict and `8` for
...
fail-closed on a retired key.

On the default (current-use) path, `verifyChain` first authenticates a retired key's signature
against that retained public material. A forged or altered receipt or checkpoint naming the key
stays `TAMPERED`. An authentic one yields `KEY_RETIRED` (CLI exit `9`), but only when every other
check passed; any other failure in the input outranks it. `KEY_RETIRED` is a refusal, never an
acceptance: `signaturesVerified` and `tailChecked` stay `false`, and it does not establish that the
signature was made before retirement. Its `reason` and one `key-retired:` warning (alongside the
...
the same versioned result and process exit from TypeScript, Python, Go, Rust, and C#. Its
`currentUse` half pins the default-purpose refusal order under the same lifecycle roots; each port
declares whether it implements lifecycle keyrings on that path, and a port that does not is still
run on every case and must refuse without reporting `KEY_RETIRED`.
